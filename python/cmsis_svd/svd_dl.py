...
INDEX_JSON_GZIP = 'index.json.gz'
INDEX_JSON_ZSTD = 'index.json.zstd'
...
LOCAL_DATA_INDEX_JSON_GZIP = os.path.join(LOCAL_DATA_DIR, INDEX_JSON+'.gz')
LOCAL_DATA_INDEX_JSON_ZSTD = os.path.join(LOCAL_DATA_DIR, INDEX_JSON+'.zstd')
...
class SvdDl:
    ...
    @staticmethod
    def _get_hash_from_index_hash(filename: Union[str, os.PathLike], target: str = 'index.json') -> str:
    ...
    def dl_pack_to_local(self, dotted_name: str):
        ...
        if 'gzip' in pack_info['files'] and not have_archive:
            ...
            try:
                ...
                try:
                    self._decompress_archive_zstd(pack_path_gzip, LOCAL_DATA_DIR, extract_files)
                finally:
                    os.unlink(pack_path_zstd)
    ...
    async def download_svd(self, download_string: str) -> None:
        ...
        if not have_index and have_zstd:
            try:
                ...
                index_json_hash_valid = self._get_hash_from_index_hash(LOCAL_DATA_INDEX_HASH, target='index.json.zstd')
        ...
        if not have_index:
            try:
                ...
                index_json_hash_valid = self._get_hash_from_index_hash(LOCAL_DATA_INDEX_HASH, target='index.json.gz')
                if index_json_hash != index_json_hash_valid:
                    raise SvdDlError(f'"index.json.gz" is corrupted.')
        ...
        print(f'[i] Validating: index.json')
        ...
        if not self._validate_index():
            raise SvdDlError(f'"index.json" contains invalid data.')
...
def main():
    ...
    if script_args.svd_dl_subparser == sub_parser_dl:
        asyncio.run(SvdDl(repo=script_args.svd_repo).download_svd(script_args.svd_resources))
